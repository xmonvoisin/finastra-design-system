...
export class WizardNavigationService implements OnDestroy {
  ...
  public checkAndCommitCurrentPage(buttonType: string) {
    ...
    let isNext: boolean, isDone: boolean;

    isNext = buttonType === 'next';
    isDone = buttonType === 'done';
  ...
  public canGoTo(pagesToCheck: UxgWizardPage[]): boolean {
    if (!pagesToCheck || pagesToCheck.length < 1) {
      return false;
    }

    if (pagesToCheck.some((page: UxgWizardPage) => page.nextStepDisabled)) {
      return false;
    }
